...
export function registerBinanceGetEthRedemptionHistory(server: McpServer) {
    server.tool(
        "binance_staking_eth_redemption_history",
        ...
        async (params) => {
            try {
                ...
                return fail("Failed to retrieve history API allows users to retrieve their historical ETH staking redemption records", error);
            }
        }
    );
}
